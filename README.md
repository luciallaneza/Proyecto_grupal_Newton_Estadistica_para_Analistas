## Correlación - Regresión:

Proyecto que forma parte del trabajo ***Proyecto_grupal_Newton*** detallado en la carpeta de este repo "02_Jupyter_grupal" del Bootcamp de Data Analyst de [Factoría F5](https://factoriaf5.org/), propuesto por la formadora [Anaïs Rodríguez](https://github.com/Anais-RV) y la co-formadora [Yeraldin Salazar](https://github.com/Yeraldsb).


Coders de este equipo de trabajo:

- [Alejandro Reyes](https://github.com/alexrey08)
- [Iván González](https://github.com/Asenjo76/) 
- [Jose Puges](https://github.com/josepuges)
- [Lucía Peláez](https://github.com/lucypelaez)
- [Lucía Llaneza](https://github.com/luciallaneza/luciallaneza)
- [Michelle Olivares](https://github.com/michelleolivares86-tech)

### *Stack Tecnológico* 🛰️
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![REST Countries API](https://img.shields.io/badge/REST_Countries_API-v3.1-4CAF50?style=for-the-badge&logo=json&logoColor=white)



## Trabajo de equipo

En nuestro equipo nos hemos encargado de investigar:

- Consumo de la API pública [REST Countries](https://restcountries.com).
- Correlación-regresión (Pearson y Spearman).   
- Regresión lineal simple.
    - Pendiente (β₁)
    - Intercepto (β₀)
    - Bondad de ajuste (R²)
- Residuos

## Punto de inicio ╰┈➤
REST Countries API   
URL: https://restcountries.com/   
Versión: v3.1   
Tipo: API REST pública y gratuita   
Endpoint utilizado:   
https://restcountries.com/v3.1/all?fields=name,area,population,region,gini   

**Datos obtenidos:**  
Nombre del país   
Región geográfica   
Población   
Área (km²)

**Uso en el proyecto:**   
 Fuente de datos principal para el análisis de correlación y regresión lineal entre área y población de una muestra de 100 países.   

**Autenticación:** No requiere API key ✅   

**Formato de respuesta:** JSON 

## Librerías 📔✏️
- numpy as np  -> Cálculos numéricos
- pandas as pd  -> Manipulación de datos
- matplotlib.pyplot as plt  -> Gráficos básicos
- seaborn as sns  ->  Gráficos estadísticos
- plotly.express as px  ->  Gráficos interactivos
- scipy from scipy import stats  ->  Funciones estadísticas